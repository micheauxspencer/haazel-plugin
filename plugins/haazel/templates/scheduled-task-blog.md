...
Create the post document using `create_documents` with ALL these fields
(`body` as Portable Text blocks, not a raw markdown string):
...
- `body`: Full article body as Portable Text
...
Upload the generated image to Sanity CDN and link it to the post's `mainImage` field using the Sanity MCP `patch_documents` tool:
